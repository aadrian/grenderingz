...
class BootStrap {
    ...
    def init = { servletContext ->
        ...
        // create data only if there's none!
        if(MyEntityOne.count() == 0 && MyEntityTwo.count() == 0 && MyEntityThree.count() == 0) {
            println "-->  ... Initializing data, since the DB was empty ..."
            ...
            println "--> ... No data init, since the DB has data .."
        }
...
*/
        println "--> Init application DONE!"
    }
    def destroy = {
        println "--> Stop application ...."
        println "--> ... No cleanup required .."
        println "--> Stop application DONE!"
    }
}
